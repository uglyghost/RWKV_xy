...
// C: the length of the content
// _w: the array w in equation (14). dim (512)
// _u: the array u in equation (14). dim (512)
// _k: k_t
// _v: v_t
// _y: output
template <typename F>
__global__ void kernel_forward(const int B, const int T, const int C,
    ...
    // blockIdx: dim of the block, also means the max number of threads in one block
    // threadIdx: the ID of thread
    const int idx = blockIdx.x * blockDim.x + threadIdx.x;
    // Calculate the batch indices and position divided by the context length
    const int _b = idx / C;    // batch indices
    const int _c = idx % C;    // the position in one batch (sentence)
    // Calculate the offset for indexing into the data
    // the position of the data in one dim array.
    const int _offset = _b * T * C + _c;

    // Get the values for u and w
    F u = _u[_c];
    F w = _w[_c];

    // Calculate the pointers for k, v, and y
    const F *__restrict__ const k = _k + _offset;  // find the position of k in one dim array
    const F *__restrict__ const v = _v + _offset;  // find the position of v in one dim array
    F *__restrict__ const y = _y + _offset;        // find the position of y in one dim array
...
__global__ void kernel_backward(const int B, const int T, const int C,
    ...
    const int _c = idx % C;
    // Calculate the offset for indexing into the data
    const int _offset = _b * T * C + _c;  // Get the values for u and w
