...
# from wkv_pytorch import WKV_core
...
class WKV(torch.autograd.Function):
    ...
    def forward(ctx, B, T, C, w, u, k, v):
        ...
        y = torch.empty_like(v)  # assuming v is of the same shape as expected output

        u_c = u.expand(12, -1)
        w_c = w.expand(12, -1)
        p = q = torch.zeros_like(u_c)
        o = torch.full_like(u_c, MIN_VALUE)

        for i in range(T):
            no = torch.max(o, u_c + k[:, i, :])
            A = torch.exp(o - no)
            B = torch.exp(u_c + k[:, i, :] - no)
            y[:, i, :] = (A * p + B * v[:, i, :]) / (A * q + B)

            no = torch.max(w_c + o, k[:, i, :])
    ...
    def backward(ctx, gy):
        ...
        p = q = dpdw = dqdw = torch.zeros_like(u_c)
        gw[:, :] = gu[:, :] = torch.zeros_like(u_c)
        o = torch.full_like(u_c, MIN_VALUE)

        y = torch.zeros_like(gy)
        z = torch.zeros_like(gy)
        zexp = torch.zeros_like(gy)

        for i in range(T):
            no = torch.max(o, k[:, i, :] + u_c)
            A = torch.exp(o - no)
            B = torch.exp(k[:, i, :] + u_c - no)
        ...
        gp = gq = torch.zeros_like(u_c)
        o = torch.full_like(u_c, MIN_VALUE)
        for i in range(T - 1, -1, -1):
            A = gy[:, i, :] * z[:, i, :] * torch.exp(zexp[:, i, :])
            B = torch.exp(k[:, i, :] + o)
            gk[:, i, :] = A * (v[:, i, :] - y[:, i, :]) + B * (gp * v[:, i, :] + gq)
            gv[:, i, :] = A + B * gp

            no = torch.max(w_c + o, zexp[:, i, :] - k[:, i, :] - u_c)
            A = torch.exp(w_c + o - no)
            B = gy[:, i, :] * z[:, i, :] * torch.exp(zexp[:, i, :] - k[:, i, :] - u_c - no)
            gp = A * gp + B
            gq = A * gq - B * y[:, i, :]
            o = no
