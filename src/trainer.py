...
torch.backends.cudnn.allow_tf32 = os.environ['RWKV_FLOAT_MODE'] != 'fp32'
torch.backends.cuda.matmul.allow_tf32 = os.environ['RWKV_FLOAT_MODE'] != 'fp32'
...
class TrainerConfig:
    ...
    def __init__(self, **kwargs):
        ...
        self.final_tokens = 0
        self.epoch_save_frequency = 0
        self.epoch_save_path = 'trained-'
...
class Trainer(LightningLite):
    ...
    def get_run_name(self):
        ...
        return f"{cfg.vocab_size}-{cfg.ctx_len}-{cfg.model_type}-{cfg.n_layer}-{cfg.n_embd}"
                ...
                def run_epoch(split):
                    ...
                    for it, (x, y) in pbar:
                        ...
                        if is_train:  # backprop and update the parameters
                            ...
                            pbar.set_description(f"miniE {epoch + 1 + self.EPOCH_BEGIN} s {self.steps} "
                                                 f"prog {progress * 100.0:.2f}% : ppl {math.exp(self.avg_loss):.6f} "
                                                 f"loss {self.avg_loss:.6f} lr {lr:e}")
                    ...
                    if self.cuda_id == 0:
                        ...
                        if (self.config.epoch_save_frequency > 0 and epoch % self.config.epoch_save_frequency == 0) or (
                            ...
                            torch.save(raw_model.state_dict(),
                                       self.config.epoch_save_path + str(epoch + 1 + self.EPOCH_BEGIN) + '.pth')
